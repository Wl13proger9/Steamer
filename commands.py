...
def restart_st():
    ...
    time.sleep(2)   
    os.system(f"start {steam_path}/steam.exe")
...
'''''
def get_name_by_id(file_path, game_id):
    try:
        game_data = {}
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip()
                if " - " in line:
                    file_id, game_name = line.split(" - ", 1)
                    game_data[file_id] = game_name
        return game_data.get(game_id, f"{no}Game with this ID not found..")
    
    except FileNotFoundError:print(f"{hm}File not found...")
    except Exception as e:print(f"{hm}Error: {e}")
'''''
