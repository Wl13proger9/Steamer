#pyinstaller steamer_lite.py --onefile --add-data "commands.py;." --add-data "data.txt;." --hidden-import=commands
...
10 /c - ставит/убирает всякие цвета
...
10 /c - enable/disable any colors
...
''',
    ...
    if lang == 1:return  ru.get(str(id)) 
    else:return eng.get(str(id))


if txt.get("steam-path") == None:txt.set("steam-path",get_steam_path())
if txt.get("hi") == None:txt.set("hi","Made by: WL13Proger9")  
if txt.get("c") == None:txt.set("c","0")  
...
''')
    ...
    quit()
if not os.path.exists("./hellyeah/"):print(f"{commands.hm}{tran(lang,'19')}")
...
if  not os.path.exists(f"{txt.get('steam-path')}/hid.dll"):repa.append("hid.dll")
if  not os.path.exists(f"{txt.get('steam-path')}/config/stUI/Steamtools.exe"):repa.append("Steamtools.exe")
if  not os.path.exists(f"{txt.get('steam-path')}/config/stplug-in/luapacka.exe"):repa.append("luapacka.exe")
if  not os.path.exists(f"{txt.get('steam-path')}/config/stplug-in/Steamtools.st"):repa.append("Steamtools.st")
if  not os.path.exists(f"{txt.get('steam-path')}/config/stplug-in/Steamtools.lua"):repa.append("Steamtools.lua")
if repa != []:
...
version_build = "18/04/25cmd"
version = "1.0 lite"
        
print(text2art("Steamer Lite"))
print(txt.get("hi")) 
    ...
    while True:
        try:
            ...
            if "/help" in world:    
                ...
                if world.startswith("/"):print(f"{commands.no}{tran(lang,'12').format(world=world)}") 
                else:
